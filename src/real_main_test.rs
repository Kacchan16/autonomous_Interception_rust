...
fn calculate_steering_direction(from: &Interceptor, to: &Target) -> (f64, f64) {
    ...
    // ----------------------------
    let terminal_dist = 400.0;      // start forcing inside this distance (tune 500..3000)
    ...
    if r < terminal_dist {
        ...
        if tv > 1e-6 {
            ...
            let offset = (0.08 * r).clamp(0.0, 200.0); // 0.08, 200
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut target = Target::new(0.0, 30.0, 5.0, 0.0);
    let mut interceptor = Interceptor::new(0.0, 0.0, 0.0, 0.0);
    let interceptor_speed = 5.55; // Constant interceptor speed (m/step)
    ...
    let collision_threshold = 1.0; // meters
